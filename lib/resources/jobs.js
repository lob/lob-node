...
(function() {
  ...
  this.create = function(params, callback) {
    ...
    var objects = params.objects, isBuffer, key;

    if(objects instanceof Array) {

      for(var i = 0, l = objects.length, obj, objCount; i < l; i++) {

        obj      = objects[i];
        objCount = 'object' + (i + 1).toString();

        params[objCount] = obj;

        if(!obj.hasOwnProperty('file')) continue;

        isBuffer = Buffer.isBuffer(obj.file);

        if(isBuffer) {
          params[objCount].file = { value: obj.file, options: { filename: 'file' + i + '.pdf' } };
        }

        if(!/^http/.test(obj.file) && !isBuffer) {
          params[objCount].file = fs.createReadStream(obj.file);
        }
      }
    ...
    for(var p in params) {
      if(!(params[p] instanceof Object)) continue;
